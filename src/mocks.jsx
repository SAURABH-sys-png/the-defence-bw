...
export default function MocksAndPYQs() {
  ...
  const filteredDownloads = downloads.filter((item) => {
    ...
    const matchesExam = selectedExam === "All" || item.exam.toUpperCase() === selectedExam.toUpperCase();
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-6 font-sans">
      ...
      <div className="text-center mb-10">
        ...
        <p className="text-slate-500 max-w-xl mx-auto mt-2 text-sm md:text-base font-light">
          Boost your NDA, CDS, and AFCAT exam preparation by downloading official previous year question papers and expert mock tests.
        </p>
      </div>
      ...
      <div className="bg-slate-50 border border-dashed border-slate-300 rounded-xl p-4 mb-8 text-center text-slate-400 text-xs hover:bg-slate-100/50 transition-colors">
        ...
        Unlock Premium NDA & CDS Test Series with video solutions. <span className="text-indigo-600 hover:underline cursor-pointer font-medium">Use Code 'DEFENCE10' &rarr;</span>
      </div>
      ...
      <div className="grid lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm space-y-4">
              ...
              <div className="flex bg-slate-100 p-1 rounded-lg border border-slate-200/40">
                <button
                  ...
                  className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${
                    selectedCategory === "All" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-600 hover:text-indigo-600"
                  }`}
                >
                  All Types
                </button>
                <button
                  ...
                  className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${
                    selectedCategory === "pyq" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-600 hover:text-indigo-600"
                  }`}
                >
                  PYQs
                </button>
                <button
                  ...
                  className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${
                    selectedCategory === "mock" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-600 hover:text-indigo-600"
                  }`}
                >
                  Mock Tests
                </button>
            ...
            <div className="flex flex-wrap gap-2 items-center pt-2 border-t border-slate-50">
              <span className="text-xs text-slate-400 font-medium mr-1 uppercase">Filter Exam:</span>
              {exams.map((exam) => (
                <button
                  key={exam}
          ...
          {filteredDownloads.length === 0 ? (
            <div className="bg-white rounded-xl border border-slate-100 p-12 text-center shadow-sm">
              ...
              <h3 className="font-bold text-slate-700 text-lg">No Papers Available</h3>
              <p className="text-slate-400 text-sm mt-1">We couldn't find matches for your search/filters.</p>
            </div>
          ) : (
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className={`p-3 rounded-xl ${
                      item.category === "pyq" ? "bg-indigo-50 text-indigo-600" : "bg-emerald-50 text-emerald-600"
                    }`}>
                      {item.category === "pyq" ? (
                    ...
                    <div className="space-y-1">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full border ${
                          ...
                          {item.category === "pyq" ? "Official PYQ" : "Practice Mock"}
                        </span>
          ...
          <div className="bg-gradient-to-r from-indigo-900 to-indigo-950 text-white rounded-2xl p-6 shadow-md border border-indigo-800/20 relative overflow-hidden flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <div className="space-y-1 z-10 text-center sm:text-left">
              <span className="bg-indigo-500/20 border border-indigo-400/20 text-indigo-300 text-[10px] uppercase font-bold tracking-widest px-2 py-0.5 rounded">Ad Promotion</span>
              <h4 className="font-bold text-base md:text-lg">Premium SSB Interview Masterclass</h4>
              <p className="text-indigo-200 text-xs font-light">Interactive classes, live OIR/PPDT sessions and mock interview loops with assessors.</p>
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div className={`transition-all duration-300 ease-in-out ${showGuide ? "max-h-[800px] border-t border-slate-100" : "max-h-0 overflow-hidden"}`}>
              <div className="p-4 space-y-4 text-xs text-slate-600 leading-relaxed bg-slate-50/50">
                <p>Follow these simple steps to add your PDF papers & mocks so users can download them:</p>
                
                <div className="space-y-3">
                  <div className="flex gap-2">
                    <span className="bg-indigo-100 text-indigo-700 font-extrabold w-5 h-5 rounded-full flex items-center justify-center shrink-0">1</span>
                    <p>
                      <strong>Move PDF file:</strong> Place your PDF file inside the React application's <code>public/pdfs/pyq/</code> or <code>public/pdfs/mocks/</code> folder.
                    </p>
                  </div>

                  <div className="flex gap-2">
                    <span className="bg-indigo-100 text-indigo-700 font-extrabold w-5 h-5 rounded-full flex items-center justify-center shrink-0">2</span>
                    <p>
                      <strong>Open configuration:</strong> Open the config file <a href="file:///home/saurabh/gitrepos/the-defence-bw/src/data/downloads.json" className="text-indigo-600 font-semibold hover:underline">src/data/downloads.json</a>.
                    </p>
                  </div>

                  <div className="flex gap-2">
                    <span className="bg-indigo-100 text-indigo-700 font-extrabold w-5 h-5 rounded-full flex items-center justify-center shrink-0">3</span>
                    <p>
                      <strong>Add a new entry:</strong> Paste a new JSON entry like this at the end:
                    </p>
                  </div>
...
{`{
  ...
  "description": "Short description of GAT exam."
                  ...
                  <div className="flex gap-2">
                    <span className="bg-indigo-100 text-indigo-700 font-extrabold w-5 h-5 rounded-full flex items-center justify-center shrink-0">4</span>
                    <p>
                      <strong>Save changes:</strong> Save the config file. The system automatically reads it, updates the search filters, and displays the download option!
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm space-y-4">
            ...
            <div className="aspect-square bg-slate-100 rounded-xl flex flex-col justify-center items-center text-center p-4 border border-slate-200/50">
              ...
              <h5 className="font-bold text-slate-800 text-sm">CDS Written Online Course</h5>
              <p className="text-slate-500 text-[11px] mt-1 font-light leading-relaxed">Prepare for CDS Math, English & GK under former UPSC examiners.</p>
